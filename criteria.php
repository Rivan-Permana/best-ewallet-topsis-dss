    ...
    if ($action === 'add') {
        ...
        if (empty($errors)) {
            try {
                ...
                redirect('criteria.php');
            } catch (Exception $e) {
                if (strpos($e->getMessage(), 'UNIQUE') !== false) {
                    $errors[] = 'Kode kriteria sudah digunakan!';
                } else {
                    $errors[] = $e->getMessage();
                }
            }
        ...
        if (empty($errors)) {
            try {
                $db->query(
                    "UPDATE criteria SET code=?, name=?, weight=?, type=?, updated_at=CURRENT_TIMESTAMP
                    WHERE id=?",
                    [$code, $name, $weight, $type, $id]
                );
                setFlash('success', 'Kriteria berhasil diupdate!');
                redirect('criteria.php');
            } catch (Exception $e) {
                if (strpos($e->getMessage(), 'UNIQUE') !== false) {
                    $errors[] = 'Kode kriteria sudah digunakan!';
        ...
        $id = intval(post('id'));
        try {
            $db->query("DELETE FROM criteria WHERE id=?", [$id]);
            setFlash('success', 'Kriteria berhasil dihapus!');
            redirect('criteria.php');
        } catch (Exception $e) {
            setFlash('error', 'Gagal menghapus kriteria: ' . $e->getMessage());
            redirect('criteria.php');
        }
    }
}
...
if ($action === 'edit' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $db->query("SELECT * FROM criteria WHERE id=?", [$id]);
    $editData = $stmt->fetch();
    if (!$editData) {
        setFlash('error', 'Data kriteria tidak ditemukan!');
        redirect('criteria.php');
    }
}
...
// Count total records
$countSql = "SELECT COUNT(*) as count FROM criteria";
if ($search) {
    $countSql .= " WHERE code LIKE ? OR name LIKE ?";
    $stmt = $db->query($countSql, ["%$search%", "%$search%"]);
} else {
...
if ($search) {
    $stmt = $db->query($sql, ["%$search%", "%$search%", $perPage, $pagination['offset']]);
} else {
...
$stmt = $db->query("SELECT SUM(weight) as total FROM criteria");
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            <h3 class="text-xl font-semibold text-gray-800">Daftar Kriteria</h3>
            <p class="text-sm text-gray-600 mt-1">Total Bobot: <strong><?php echo formatNumber($totalWeight); ?></strong></p>
        </div>
        <button onclick="showModal('addModal')" class="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition flex items-center">
    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
        <form method="GET" class="flex gap-2">
            ...
            <?php if ($search): ?>
            <a href="criteria.php" class="bg-gray-500 text-white px-6 py-2 rounded-lg hover:bg-gray-600 transition">
                    ...
                    <?php else: ?>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <span class="font-semibold text-blue-600"><?php echo formatNumber($item['weight']); ?></span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <?php echo getCriteriaTypeBadge($item['type']); ?>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <a href="criteria.php?action=edit&id=<?php echo $item['id']; ?>"
        ...
        <form method="POST" class="space-y-4">
            ...
            <div>
                ...
                <input type="text" name="name" required maxlength="100"
                       ...
                       placeholder="Contoh: Kemudahan Penggunaan"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Bobot (0-1)</label>
                <input type="number" name="weight" required step="0.0001" min="0" max="1"
                       value="<?php echo $editData ? $editData['weight'] : ''; ?>"
                       placeholder="Contoh: 0.2567"
                       class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                <p class="text-xs text-gray-500 mt-1">Total bobot semua kriteria harus = 1</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Tipe Kriteria</label>
                <select name="type" required
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="">-- Pilih Tipe --</option>
                    <option value="benefit" <?php echo ($editData && $editData['type'] === 'benefit') ? 'selected' : ''; ?>>
                        Benefit (Semakin tinggi semakin baik)
                    </option>
                    <option value="cost" <?php echo ($editData && $editData['type'] === 'cost') ? 'selected' : ''; ?>>
